...
void setup() {
  ...
  rightFrontServo.attach(ipn[0]);
  leftFrontServo.attach(ipn[1]);
  leftBackServo.attach(ipn[2]);
  rightBackServo.attach(ipn[3]);
...
void loop() {
  ...
  if (Serial.available() > 0){
    ...
    if(opBuffer[0] == 0){
       if (Serial.available() >= wheelCommandBufferSize*2) {
